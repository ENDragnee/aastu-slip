...
interface LaptopBatchModalProps {
  isOpen: boolean;
  onClose: () => void;
}
...
export function LaptopBatchModal({ isOpen, onClose }: LaptopBatchModalProps) {
  ...
  const downloadTemplate = () => {
    const csvContent = "data:text/csv;charset=utf-8,studentId,serialNumber,manufacturer,model\nETS1234/14,SN12345,DELL,XPS 15";
    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", "laptop_template.csv");
  ...
  const mutation = useMutation({
    mutationFn: async (rows: CsvRow[]) => {
      ...
      // 3. Send via FormData (matching your existing API)
      const formData = new FormData();
      formData.append("file", file);

      const res = await axios.post("/api/laptops", formData);
      return res.data;
    },
    onSuccess: (data: ApiResponse) => {
      ...
      if (data.success && data.failed === 0) {
        toast({ title: "Success", description: `Processed ${data.processed} laptops successfully.` });
  ...
  const parseFile = (file: File) => {
    Papa.parse<CsvRow>(file, {
      ...
      complete: (results) => {
        ...
        // Basic validation
        if (!headers.includes("studentId") || !headers.includes("serialNumber")) {
          toast({
            title: "Invalid CSV",
            description: "Missing required columns: studentId, serialNumber",
            variant: "destructive"
          });
          return;
        }
      ...
      <DialogContent className="max-w-4xl h-[90vh] flex flex-col p-0 gap-0">
        ...
        <DialogHeader className="p-6 pb-2">
          <div className="flex justify-between items-start">
            <div>
              <DialogTitle>Bulk Laptop Registration</DialogTitle>
              <DialogDescription>
                Upload and edit laptop data before processing.
              </DialogDescription>
            </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-2">
          ...
          {data.length === 0 && !uploadResult && (
            ...
            >
              ...
              <p className="text-xs text-muted-foreground mt-1">studentId, serialNumber, manufacturer, model</p>
            </div>
          )}
        ...
        <DialogFooter className="p-6 pt-2 border-t bg-muted/10">
          ...
          {/* Show Process button only if data exists and results are not yet shown (or mixed results) */}
          {data.length > 0 && (
