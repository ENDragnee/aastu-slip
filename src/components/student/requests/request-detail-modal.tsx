...
import { QRCodeSVG } from "qrcode.react"; // You might need: npm install qrcode.react
import {
                ...
                <ul className="space-y-2">
                  {request.laptops.map((l, idx) => (
                    <li key={idx} className="text-sm bg-muted/50 p-2.5 rounded-md border">
                      <div className="font-medium">{l.laptop.manufacturer} {l.laptop.model}</div>
                      <div className="text-xs text-muted-foreground font-mono mt-0.5">SN: {l.laptop.serialNumber}</div>
                    </li>
                  ))}
                </ul>
              ) : (
